...
async function showOrders(ctx: any) {
  ...
  const orders = await prisma.order.findMany({
    ...
    include: {
      route: {
        include: {
          fromRegion: true, fromDistrict: true,
          toRegion: true, toDistrict: true,
        },
      },
  ...
  for (const order of orders) {
    const from = order.route.fromDistrict?.name ?? order.route.fromRegion.name;
    const to = order.route.toDistrict?.name ?? order.route.toRegion.name;
    const icon = order.status === "ACTIVE" ? "🟢" : "✅";
    const offerBadge = order._count.offers > 0 ? ` (${order._count.offers} taklif)` : "";
    kb.text(`${icon} ${from} → ${to}${offerBadge}`, `order_offers:${order.id}`).row();
  }
